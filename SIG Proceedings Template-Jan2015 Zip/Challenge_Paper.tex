...
\title{Feature Usage In Java\titlenote{(Produces the permission block, and
...
% 1st. author
\alignauthor
Michael Feist\titlenote{Mike note.}\\
       \affaddr{University of Alberta}\\
       \email{mdfeist@ualberta.ca}
% 2nd. author
\alignauthor
Ian Watts\titlenote{Ian Note.}\\
       \affaddr{University of Alberta}\\
...
\maketitle
\begin{abstract}
This paper examines the usage of language features in Java.
A tool for computing the diff of two Abstract Syntax Trees is used to
compare contributions of different authors to a repository. From the differences,
a histogram of language features is computed for each user. A comparison between users
is used to evaluate the ammount of language features they use as well as
 suggest an experience level of that user.
...
there are millions of places features could potentially be used but weren’t; developers convert existing code to use new features; and we found thousands of instances of potential resource handling bugs.
Are the use of languages anticipated? Are they being used before the release date?
